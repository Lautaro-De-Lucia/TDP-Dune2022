    ...
    //  loopeamos infinitamente
        ...
        switch (command){
            ...
            case MOUSE_SELECTION:
                ...
                break;
            default:
                this->protocol.send_idle(this->socket);
                break;
        }

        response_t res;
        this->protocol.receive_command_response(res,this->socket);
        this->print(usr_msg[res], DATA_PATH FONT_IMPACT_PATH, 200, 300, 10, res >= 0 ? colors[GREEN] : colors[RED], 1000);
...
void Player::update(std::vector<State>& server_data, int spice, int energy){
